...
internal class AidAnotherSave : UnitFactComponentDelegate, IRulebookHandler<RuleSavingThrow>, IInitiatorRulebookHandler<RuleSavingThrow>, ISubscriber, IInitiatorRulebookSubscriber, IUnitNewCombatRoundHandler, IUnitCombatHandler, IUnitBuffHandler
{
    ...
    public void OnEventAboutToTrigger(RuleSavingThrow evt)
    {
        ...
        var rankFeature = BlueprintTool.Get<BlueprintFeature>(AidAnotherFeature.SaveRank.Guid);
        ...
        evt.AddTemporaryModifier(evt.Initiator.Stats.SaveReflex.AddModifier(bonus, Fact, ModifierDescriptor.UntypedStackable));
        evt.AddTemporaryModifier(evt.Initiator.Stats.SaveFortitude.AddModifier(bonus, Fact, ModifierDescriptor.UntypedStackable));
    }
    ...
    public void OnEventDidTrigger(RuleSavingThrow evt)
    {
        ...
        {
            if (base.Context.MaybeCaster == buff.Context.MaybeCaster && buff.Blueprint == BlueprintTool.Get<BlueprintBuff>(AidAnotherFeature.SaveBuff.Guid))
            {
                buff.Remove();
            }
        }
    ...
    private void RemoveBuffFromUnit(UnitEntityData unit)
    {
        if (unit == Context.MaybeCaster)
        {
            ...
            foreach (var buff in buffs)
            {
                if (buff.Context.MaybeCaster == unit && buff.Blueprint == BlueprintTool.Get<BlueprintBuff>(AidAnotherFeature.SaveBuff.Guid))
                {
                    buff.Remove();
                    break;
                }
    ...
    public void HandleBuffDidAdded(Buff buff)
    {
        ...
        {
            if (currentBuff.Context.MaybeCaster == buff.Context.MaybeCaster && buff != currentBuff && currentBuff.Blueprint == BlueprintTool.Get<BlueprintBuff>(AidAnotherFeature.SaveBuff.Guid))
            {
                currentBuff.Remove();
                break;
            }
